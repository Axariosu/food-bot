...
class woPCBA(commands.Cog):
    ...
    async def on_ready(self):
        print('cog.wopcba successfully loaded!')
        
    @commands.command(aliases=['wopcba'])
    async def start_wopcba(self, ctx, *args):
        res = discord.Embed(title="Starting !woPCBA", color=util.generate_random_color())
        res.add_field(name="Rules", inline=False, value="Players have some time per round to type the letters in the word I display in reverse alphabetical order!\n")
        ...
        self.context = ctx
        self.trackedPlayers = {}
        await self.wopcba_on(ctx)

    @commands.command()
    async def stop_wopcba(self, ctx):
        res = discord.Embed(title="!woPCBA Over!", color=util.generate_random_color())
        await ctx.send(embed=res)
        self.trackedPlayers = {}
        self.round = 0
        self.timer = 10e22
